    ...
    if areatype == 'ltla':
        ...
        if placegiven == '*':
            if dategiven == '*':
                ...
                queryCreated = dbss.utla_cases.find({ "areaType" : areatype }, { '_id' : False }).sort("date", -1).limit(limitAmountltla)
            else:
                # Search by just date
                # Get 100 results for the parameter given (this is because there would be too many to return for this one) (for this areatype)
                queryCreated = dbss.utla_cases.find({ "date" : start }, { '_id' : False }).limit(limitAmountltla)

        # If a place name was given
        else:
            # If no date was given, search for just the areaname (for this areatype)
            if dategiven == '*':
                queryCreated = dbss.utla_cases.find({ "areaName" : re.compile(placegiven, re.IGNORECASE) }, { '_id' : False }).limit(limitAmountltla)
            # If no date was given, search for just the areaname (for this areatype)
            else:
                # If no date was given, search for the date and areaname (for this areatype)
                queryCreated = dbss.utla_cases.find({ "date" : start, "areaName" : re.compile(placegiven, re.IGNORECASE) }, { '_id' : False }).limit(limitAmountltla)
    ...
    # If the area given was utla
        ...
        if placegiven == '*':
            if dategiven == '*':
                # Search by just area
                # Get the results for just this parameter given (this is because there would be too many to return for this one) (for this areatype)
                queryCreated = dbss.nations_cases.find({ "areaType" : areatype }, { '_id' : False }).sort("date", -1).limit(limitAmount)
            else:
                # Search by just date
                # Get 100 results for the parameter given (this is because there would be too many to return for this one) (for this areatype)
                queryCreated = dbss.nations_cases.find({ "date" : start }, { '_id' : False }).limit(limitAmount)

        # If a place name was given
        else:
            # If no date was given, search for just the areaname (for this areatype)
            if dategiven == '*':
                queryCreated = dbss.nations_cases.find({ "areaName" : re.compile(placegiven, re.IGNORECASE) }, { '_id' : False }).limit(limitAmount)
            # If no date was given, search for just the areaname (for this areatype)
            else:
                # If no date was given, search for the date and areaname (for this areatype)
                queryCreated = dbss.nations_cases.find({ "date" : start, "areaName" : re.compile(placegiven, re.IGNORECASE) }, { '_id' : False }).limit(limitAmount)
